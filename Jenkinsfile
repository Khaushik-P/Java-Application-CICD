pipeline{
    ...
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-100-27-195-26.compute-1.amazonaws.com:8080/job/Java Application CD/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
